	...
	// On récupère ce dont on a besoin + les id.
		...
		public ArticleSold Select_article(int articleNum) {
			...
			if (rs.next ())
			{
				...
				article = new ArticleSold (rs.getInt(1),rs.getString(2),rs.getString(3),rs.getTimestamp(4).toLocalDateTime(), rs.getTimestamp(5).toLocalDateTime(),rs.getInt(6),rs.getInt(7), category, user, auction, withdrawal);
			}
			// On récupère le TimeStamp que l'on convertit ensuite en LocalDateTime pour l'ajouter dans le constructeur.
